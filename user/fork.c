...
fork(void)
{
	...
	if (newenvid != 0) {
        ...
        int pagePerPDMAP = PDMAP / BY2PG;
        int j;
        for (i = 0; i <= VPN(USTACKTOP); i += pagePerPDMAP) {
            if ((((Pde *)(*vpd))[(i >> 10)] & PTE_V) != 0) {
                for (j = 0; j < pagePerPDMAP && i + j < VPN(USTACKTOP); j++) {
                    if ((((Pte *)(*vpt))[i + j] & PTE_V) != 0) {
                        duppage(newenvid, i + j);
                    }
                }
            }
        }
